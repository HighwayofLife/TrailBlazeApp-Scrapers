    ...
    def extract_address_from_html(
        ...
        if not api_url:
            if not config.LLM_API_ENDPOINT or not config.MODEL_ID:
                logger.warning(
                    "Neither LLM_API_URL nor both LLM_API_ENDPOINT and MODEL_ID are configured. Skipping LLM extraction."
                )
                return None
        ...
        last_exception: Optional[Exception] = LLMAPIError(
            "LLM request failed after all retries."
        )  # Initialize with a default error
        ...
        for attempt in range(config.LLM_MAX_RETRIES):
            try:
                ...
                if "promptFeedback" in response_data and response_data[
                    "promptFeedback"
                ].get("blockReason"):
                    block_reason = response_data["promptFeedback"]["blockReason"]
                    logger.warning(
                        f"Gemini response blocked due to content moderation: {block_reason}"
                    )
                ...
                if not llm_output_text:
                    logger.error(
                        f"Could not extract text from Gemini response: {response_data}"
                    )
                ...
                try:
                    ...
                    if not expected_keys.issubset(extracted_data.keys()):
                        logger.warning(
                            f"LLM JSON missing expected keys: {extracted_data}"
                        )
                    ...
                    logger.info(
                        f"Successfully extracted address via LLM: {extracted_data}"
                    )
                    return extracted_data

                except json.JSONDecodeError as e:
                    logger.error(
                        f"Failed to parse JSON from LLM response: {llm_output_text}",
                        exc_info=True,
                    )
                ...
                logger.warning(
                    f"LLM API request failed (attempt {attempt + 1}/{config.LLM_MAX_RETRIES}): {e}"
                )
                ...
                if attempt < config.LLM_MAX_RETRIES - 1:
                    ...
                    if isinstance(last_exception, Exception):
                        ...
                        raise LLMAPIError(
                            "LLM API request failed after multiple retries (unknown error)."
                        ) from e

            except (
                json.decoder.JSONDecodeError
            ) as e:  # Handle JSON decode errors separately
                logger.error(
                    f"Failed to decode JSON from LLM API response (attempt {attempt + 1}): {e}",
                    exc_info=True,
                )
                ...
                logger.error(
                    f"Unexpected error during LLM interaction (attempt {attempt + 1}): {e}",
                    exc_info=True,
                )
                ...
                if attempt < config.LLM_MAX_RETRIES - 1:
                    ...
                    logger.error(
                        "Unexpected error persisted after retries during LLM interaction."
                    )
                    # Raise a generic error wrapping the last exception
                    raise LLMAPIError(
                        "An unexpected error occurred during LLM interaction."
                    ) from last_exception
        ...
        if isinstance(last_exception, Exception):
            ...
            raise LLMAPIError(
                "LLM extraction failed after all retries (default error)."
            )
...
if __name__ == "__main__":
    ...
    if not settings.LLM_API_KEY:
        ...
        print(
            "Skipping live LLM test. Create/check .env file or set environment variables."
        )
    elif not settings.LLM_API_URL and (
        not settings.LLM_API_ENDPOINT or not settings.MODEL_ID
    ):
        print(
