...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="TrailBlazeApp-Scrapers - Event data scraper for endurance rides"
    )
    parser.add_argument(
        "--validate",
        action="store_true",
        help="Validate database operations after scraping",
    )
    parser.add_argument(
        "--no-db",
        action="store_true",
    ...
    parser.add_argument(
        "--sample",
        action="store_true",
        help="Use sample HTML file instead of live website",
    )
    parser.add_argument(
        "--sample-file",
        type=str,
...
def main() -> Dict[str, Any]:
    ...
    try:
        ...
        for scraper_name in scrapers_to_run:
            ...
            if not scraper_info:
                logger.warning(
                    f"Scraper '{scraper_name}' not found in registry. Skipping."
                )
                continue
            ...
            if db_manager:
                db_manager.scraper = (
                    scraper  # Link current scraper to DB manager for metrics
                )
            ...
            sample_file = (
                args.sample_file if args.sample_file else scraper_info["sample_file"]
            )
            ...
            try:
                if use_sample:
                    ...
                    if not os.path.exists(sample_file):
                        logger.error(
                            f"Sample file not found for {scraper_name}: {sample_file}"
                        )
                    ...
                    scrape_url = (
                        args.url
                        if args.url
                        else getattr(settings, default_url_setting, None)
                    )
                    if not scrape_url:
                        logger.error(
                            f"Scraping URL not configured for {scraper_name} (checked Settings.{default_url_setting} and --url arg). Skipping."
                        )
                        continue

                    logger.info(
                        f"Starting {scraper_name.upper()} scrape from {scrape_url}..."
                    )
                ...
                logger.info(
                    f"Scraper {scraper_name.upper()} finished. Found {len(events)} final events."
                )
                ...
                if db_manager:
                    ...
                    logger.info(
                        f"Processing {len(events)} events from {scraper_name.upper()} for database storage..."
                    )
                    ...
                    for _, event_data in events.items():
                        try:
                            ...
                            if args.validate and data_validator:
                                success, errors = (
                                    data_validator.validate_database_operation(
                                        event_data
                                    )
                                )
                            ...
                            logger.error(
                                f"Error processing event {event_data.get('source', 'unknown')}-{event_data.get('ride_id', 'unknown')}: {str(e)}"
                            )

                    logger.info(
                        f"Successfully processed {updated_event_count} DB operations for {scraper_name.upper()} events"
                    )

                    if args.validate:
                        if validation_errors:
                            logger.warning(
                                f"Validation found {len(validation_errors)} errors for {scraper_name.upper()}"
                            )
                            ...
                            logger.info(
                                f"All database operations validated successfully for {scraper_name.upper()}"
                            )
        ...
        logger.info(
            f"--- All scraping finished. Total unique events processed: {len(all_scraped_events)} ---"
        )
        return all_scraped_events

    except (FileNotFoundError, KeyError, ValueError, TypeError) as e:
        logger.critical(
            f"Unhandled error in main execution: {str(e)}"
        )  # Use critical for top-level errors
        raise
